...
$J_x^{(a,b)}, J_y^{(a,b)}$ have the Banded-Block-Banded\sotodo{Define Banded-Block-sBanded} structures
\begin{align}
...
\end{pmatrix}.
\end{align}
where we note \(P^{(a,b)}_{0,0}(x,y) \equiv H_0^{(a,b)} \: P_0^{(b,b)}\), and for each $n = 0,1,2\dots$:\sotodo{Remove $A^z_n$?}
\begin{align}
A_n &:= \begin{pmatrix}
		A^x_n \\
		A^y_n \\
...
For each $n = 0,1,2\dots$ let $\Dnt$ be any matrix that is a left inverse of $A_n$, i.e. such that $\Dnt A_n = I_{n+3}$\sotodo{n+1?}. Multiplying our system by the preconditioner matrix that is given by the block diagonal matrix of the $\Dnt$'s, we obtain a lower triangular system \cite[p78]{dunkl2014orthogonal}, which can be expanded to obtain the recurrence:
\begin{align}
\begin{cases}
\bigPab_{-1}(x,y) := 0 \\
...
\begin{figure}
	...
	\end{subfigure}
	\centering
    	\caption{\sotodoinline{Fix titles} "Spy" plots of the different operator matrices for the Laplacian operator $\Delta$, as well as the operator matrix for the Helmholtz equation, showing their sparsity. Top Left: $\laplacewii$. Top Right: $\laplacewtt$. Bottom Left: $\laplaceoo$. Bottom Right: the weighted variable coefficient Helmholtz operator $\laplacewii + k^2 \: T^{(0,0)\to(1,1)} \: V({J_x^{(0,0)}}^\top, {J_y^{(0,0)}}^\top) \: T_W^{(1,1)\to(0,0)}$ for $v(x,y) = xy^2$ and $k = 0.25$.}
        \label{fig:sparsity}
\end{figure}
...
\]
to coefficients in the non-weighted space $\bigPii(x,y)$.
...
The sparsity of the these operators can be shown using a simple integration by parts argument. \sotodoinline{Add argument}
...
\end{align}
where we define $\beta_{-1}^{(a,b)} = 0$ and $H_{-1}^{(a,b)}(x) \equiv 0$. We use a variant of the Stieltjes algorithm where the polynomials are expressed as Chebyshev polynomial expansions and the inner products are calculated via Clenshaw--Curtis quadrature. This has the benefit that it is easier to incorporate high-precision arithmetic, which we use to overcome ill-conditioning present when $b$ is large, as required for large $n$ in \eqref{eq:diskpolys}. The ApproxFun.jl \cite{ApproxFun} package gives a convenient way to manipulate Chebyshev and Jacobi expansions, which we use to calculate the inner products and norms in this algorithm.

\begin{algorithm}
\sotodo{This is just Stieltjes right? We can probably delete it and reference Gautschi.}Inputs: max degree $N$, weight $w(x)$
\begin{flalign*}
	&\text{Define } H_0(x) \equiv \frac{1}{\norm{1}_w} \\
	&\text{Define } v(x) = x  H_0(x) \\
	&\text{Set } \alpha_0 = \ip<v, H_0>_w \\
	&\text{Define } \tilde{v}(x) = v(x) - \alpha_0 H_0(x) \\
	&\text{Set } \beta_0 = \norm{\tilde{v}}_w \\
	&\text{Define } H_1(x) = \frac{\tilde{v}(x)}{\beta_0} \\
	&\text{FOR } n = 1:N \\
		& \quad \quad \text{Define } v(x) = x H_n - \beta_{n-1} H_{n-1} \\
		& \quad \quad \text{Set } \alpha_n = \ip<v, H_n>_w \\
		& \quad \quad \text{Define } \tilde{v}(x) = v(x) - \alpha_n H_n(x) \\
		& \quad \quad \text{Set } \beta_n = \norm{\tilde{v}}_w \\
		& \quad \quad \text{Define } H_{n+1}(x) = \frac{\tilde{v}(x)}{\beta_n} \\
	&\text{END }
\end{flalign*}
...
Using the quadrature rule detailed in Section 4.2, we can calculate exactly the coefficients $f_{n,k}$: \sotodo{Add sentence explaining how symmetry is imprtant here}
\begin{align}
...
\end{align}
where we note that $\norm{H_n^{(a,b)}}_{w_H^{(a,b)}} = \norm{H_m^{(a,b)}}_{w_H^{(a,b)}}$ \sotodo{Aren't these $= 1$ since we orthonormalize?} $\forall m, n = 0, 1, 2, \dots$. 
...
Find \(u(x,y)\) given a function \(f(x,y)\) such that:\sotodo{I assume there should be an $f$ in the right-hand side?}
\begin{align}
	\begin{cases}
...
\bigP^{(2,2)}.
$
Note that any function expanded in the $\bigW^{(2,2)}$ basis will satisfy both zero Dirichlet and zero Neumann boundary conditions on $\Omega$. The matrix operator for the Biharmonic operator is then given by \sotodo{Move up to other discussion on constructing Biharmonic}
\begin{align}
	\biharmonic = \laplaceoo \: \laplacewtt.
\end{align}
In Figure \ref{fig:solution-biharmonic} we see the solution to the Biharmonic equation (\ref{eqn:biharmonic}) in the half-disk $\Omega$.
...
We can note that by setting the parameter $c = 0$, and $\alpha = 0$, $\beta = 1$ we obtain the 2D orthogonal polynomials on the half-disk (note again also that we made the adjustment that $w^{(a,b,c)}_H(x) = (\beta - x)^a \: (x - \alpha)^{b} \: \rho(x)^{2c}$ for the half-disk case, for ease of writing and computation).
...
(\gamma, \delta) &:= (0,1) \\
\rho(x) &:= 1-\half x \\
w^{(a,b,c)}_H(x) &:= (\beta - x)^a \: (x - \alpha)^{b} \: \rho(x)^{c} = (1-x)^{a} \: x^b \: (1-\half x)^{c} \\
w^{(a,b)}_P(x) &:= (\delta-x)^a \: (x - \gamma)^b = (1-x)^a \: x^b.
\end{cases}
\end{align}
The weight $w^{(a,b)}_P(x)$ is the weight for the shifted Jacobi polynomials on the interval $[0,1]$, and hence the corresponding OPs are the shifted Jacobi polynomials $\{\tilde{P}_n^{(a, b)}\}$. We note that the shifted Jacobi polynomials relate the the normal Jacobi polynomials by the relationship $\tilde{P}_n^{(a,b)}(x) = P_n^{(a,b)}(2x-1)$ for any degree $n = 0,1,2,\dots$ and $x \in [0,1]$. $w^{(a,b,c)}_H(x)$ is the (non-classical) weight for the OPs we dentote $\{H_n^{(a,b,c)}\}\). Thus we arrive at the four-parameter family of 2D orthogonal polynomials $\{\Pnk^{(a,b,c,d)}\}$ on $\Omega$ given by, for \(0 \le k \le n, \: n = 0,1,2,\dots,\)
\begin{align}
	\Pnk^{(a,b,c,d)}(x,y) := H_{n-k}^{(a, b, c+d+2k+1)}(x) \: \rho(x)^k \: \tilde{P}_k^{(d,c)}(\frac{y}{\rho(x)}), \quad (x,y) \in \Omega, 
\end{align}
orthogonal with respect to the weight
\begin{align}
	W^{(a,b,c,d)}(x,y) &:= w^{(a,b,c+d)}_H(x) \: w^{(d,c)}_P(\frac{y}{\rho(x)}) \nonumber \\
