...
\end{cases}
\end{align*}
and define the associated inner products by: \bstodo{Is it best to define these ip s with the normalisation?}
\begin{align}
...
\end{align*}
where
$\ip< f, \: g >_W^{(a,b,c,d)} := \iint_\Omega \: f(x,y) \: g(x,y) \: W^{(a,b,c,d)}(x,y) \: \D y \: \D x$.
\end{definition}
...
The two 1D families of orthogonal polynomials we have denoted $\{\genjac_{n}^{(a,b)}\}$ and $\{P_n^{(b,b)}\}$ are orthonormal, with respect to the inner products respectively given by:
\begin{align*}
...
\end{align*}
which is zero for $m < n-1$. Finally, we simply have that \bstodo{Make sure correct ip definition is being used here}
\begin{align*}
...
\bstodoinline{N=100, k=200 in the example isn't converging to a solution, so need a new fig. Add in solution block norms fig  - what should RHSs be?}
...
\bstodoinline{Add in solution block norms fig - what should RHSs be?}
