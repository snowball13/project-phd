...
This provides us with a family of orthogonal polynomial spaces on the half disk. Note that we can obtain OP families on other disk-splices and rhombi by appropriately choosing the functions and constants in (2). \bstodo{Should I add in the rhombi / other disk splices formulations here?}
...
We can develop a quadrature rule on \(\Omega\) in order to evaluate (40) and obtain the coefficients vector \(\mathbf{f}\) as follows. \bstodo{Should I add in the rhombi / other disk splices formulations here?}
...
\end{align}
The sparsity of the differential operators can be shown using a simply integration by parts argument, again showing that the sparsity for differential operators is not dependent on the domain.


%
\section{P-finite element methods using sparse operators}
\bstodoinline{Need to discuss this}
...
Let $\FEset$ be the set of elements $\element$ that make up our finite element discretisation of the domain, where each $\element$ is a rhombus for example. We can choose our finite dimensional space $V_h = \{v_h \in V \quad | \quad deg(v_h)|_\element \le p \: \forall \element \in \FEset\}$.
...
We can choose a basis for $V_h$ by using the orthogonal polynomials 
\begin{align}
\bigP(x,y) := \begin{bmatrix}
		\bigP_0(x,y) \\
		\hline
...
\end{align}
Appropriately relabelling these OPs as the set $\{\phi_1,\dots,\phi_N\}$, we can define the global stiffness matrix $A$ by $A_{ij} =  a(\phi_i, \phi_j)$. The global stiffness matrix can be computed using the element stiffness matrices $A^\element = \int_\element \nabla \bigP \cdot \nabla \bigP \: d\mathbf{x}, \: \element \in \FEset$. Then we have the linear system for our finite element problem:
\begin{align}
A\mathbf{u} = \mathbf{f}
\end{align}
where the vectors $\mathbf{u}, \mathbf{f}$ are the coefficient vectors for expansion of the solution $u_h$ and for the function $f$ respectively.

Using the quadrature rule we have formulated for integrals on a rhombus element $\element$, and the sparse operator matrices for divergence (partial derivatives), we obtain a sparse global stiffness matrix.
...
\bstodo{Struggling to get plotting working}
