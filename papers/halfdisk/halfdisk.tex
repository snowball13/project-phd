...
\ \(\{\hdopnkab\}\) given by, for \(0 \le k \le n, \: n = 0,1,2,\dots,\)
...
\end{align}
where we note that \(\gamma_n^{(b)} = 0\), for \( n = 0,1,2,\dots\). We can use these to determine the 2D recurences for $\hdopnkab(x,y)$:
...
\betaab_{n,k,1} &:= {\delta_{k-1}^{(b)}}{|| \hdop_{n-1,k-1}^{(a,b)} ||_{W^{(a,b)}}^{-2}} \: \ip<\genjacnmk^{(a, b+k+1/2)}, \genjacnmk^{(a, b+k-1/2)}>_{\genjacw^{(a, b+k+1/2)}} \\
\betaab_{n,k,2} &:= {\delta_{k}^{(b)}}{|| \hdop_{n-1,k+1}^{(a,b)} ||_{W^{(a,b)}}^{-2}} \: \ip<\genjacnmk^{(a, b+k+1/2)}, \genjac_{n-k-2}^{(a, b+k+{3/2})}>_{\genjacw^{(a, b+k+{3/2})}} \\
\betaab_{n,k,3} &:= {\delta_{k-1}^{(b)}}{|| \hdop_{n,k-1}^{(a,b)} ||_{W^{(a,b)}}^{-2}} \: \ip<\genjacnmk^{(a, b+k+1/2)}, \genjac_{n-k+1}^{(a, b+k-1/2)}>_{\genjacw^{(a, b+k+1/2)}} \\
\betaab_{n,k,4} &:= {\delta_{k}^{(b)}}{|| \hdop_{n,k+1}^{(a,b)} ||_{W^{(a,b)}}^{-2}} \: \ip<\genjacnmk^{(a, b+k+1/2)}, \genjac_{n-k-1}^{(a, b+k+{3/2})}>_{\genjacw^{(a, b+k+{3/2})}} \\
\betaab_{n,k,5} &:= {\delta_{k-1}^{(b)}}{|| \hdop_{n+1,k-1}^{(a,b)} ||_{W^{(a,b)}}^{-2}} \: \ip<\genjacnmk^{(a, b+k+1/2)}, \genjac_{n-k+2}^{(a, b+k-1/2)}>_{\genjacw^{(a, b+k+1/2)}} \\
\betaab_{n,k,6} &:= {\delta_{k}^{(b)}}{|| \hdop_{n+1,k+1}^{(a,b)} ||_{W^{(a,b)}}^{-2}} \: \ip<\genjacnmk^{(a, b+k+1/2)}, \genjacnmk^{(a, b+k+{3/2})}>_{\genjacw^{(a, b+k+{3/2})}}. 
\end{align}
where $|| p(x,y) ||_{W^{(a,b)}}^2 := \int_\Omega p(x,y)^2 \: \Wab(x,y) \: d\mathbf{x}$ and $\ip<p(x), q(x)>_{\genjacw^{(a, b)}} := \int_\alpha^\beta p(x) \: q(x) \: x^a \: (1-x^2)^b \: dx$, i.e the 1D inner product using the $\{H^{(a,b)}_n\}$ family weight of $\genjacw^{(a, b)}$.

\end{lemma}

\begin{proof}
Note that \(\ip<y \hdopnkab, \hdopmjab>_\Wab = 0\) for \(m < n-1\) (and for \(m > n+2\)). Thus for \(m = n-1, n, n+1,\) \(j = 0,\dots,m:\)
\begin{align}
...
&= \begin{cases}
    	\delta_k^{(b)} \: \ip<\genjacnmk^{(a, b+k+\half)}, \genjac_{m-k-1}^{(a, b+k+\frac{3}{2})}>_{\genjacw^{(a, b+k+\frac{3}{2})}} \quad& \text{if } j = k+1 \\
	\delta_{k-1}^{(b)} \: \ip<\genjacnmk^{(a, b+k+\half)}, \genjac_{m-k+1}^{(a, b+k-\half)}>_{\genjacw^{(a, b+k+\half)}} \quad& \text{if } j = k-1 \\
	0 & \text{otherwise}
      \end{cases}.
\end{align}
...
\begin{align*}
c_{m,j} = \frac{\ip<\pddx \hdopnkab, P^{(a+1, b+1)}_{m,j} >_{W^{(a+1, b+1)}}}{\norm{P^{(a+1, b+1)}_{m,j}}^2_{W^{(a+1, b+1)}}}.
\end{align*}
Second, note that
\begin{align}
...
\begin{align}
	&\ip<\pddx \hdopnkab, P^{(a+1, b+1)}_{m,j} >_{W^{(a+1, b+1)}} \nonumber \\ 
...
\begin{align*}
c_{n-1,k-1} = \frac{\ip<\pddy \hdopnkab, P^{(a, b+1)}_{n-1,k-1} >_{W^{(a, b+1)}}}{\norm{P^{(a, b+1)}_{n-1,k-1}}^2_{W^{(a, b+1)}}}
\end{align*}
where
\begin{align}
	\ip<\pddy \hdopnkab, P^{(a, b+1)}_{n-1,k-1} >_{W^{(a, b+1)}} = \norm{\genjac_0^{(a,b+k+\half)}}^2_{\genjacw^{(a, b+k+\half)}} 
		\: \ip< P_k^{(b,b) \: \prime}, \: P_{k-1}^{(b+1,b+1)} >_{w_P^{(b+1)}}
\end{align}
where we note that $\norm{\genjac_n^{(a,b)}}_{\genjacw^{(a,b)}} = \norm{\genjac_m^{(a,b)}}_{\genjacw^{(a,b)}}$ $\forall m, n = 0, 1, 2, \dots$, and thus is in fact given by the integral of the weight function $\genjacw^{(a,b)}$ over $(0,1)$. 
