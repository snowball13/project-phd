...
Three-term recurrences lead to Jacobi operators that correspond to multiplication by $x$ and $y$. Define, for $n=0,1,2,\dots$: \sotodo{Why bmatrix, not pmatrix?}\bstodo{No reason, I am not familiar with pmatrix}
\begin{align}
...
\bigPab := \begin{bmatrix}
	...
	\end{bmatrix}
\end{align}
\sotodo{Why not just make $\bigPab$ infinite for now?}and set $J_x^{(a,b)}, J_y^{(a,b)}$ as the Jacobi matrices corresponding to
\begin{align}
...
A^x_n &:= \begin{bmatrix}
		...
		& \ddots & & \vdots & \\
		& & \alphaab_{n+1,n,1} & 0 \\
	    \end{bmatrix} \in \R^{(n+1)\times(n+2)}, \quad n = 0,1,2,\dots \\
B^x_n &:= \begin{bmatrix}
		\alphaab_{n,0,2} & & \\
		& \ddots & \\
		& & \alphaab_{n,n,2} \\
	    \end{bmatrix} \in \R^{(n+1)\times(n+1)} \quad n = 0,1,2,\dots \\
...
C^y_n &:= \begin{bmatrix}
		...
		& & \ddots & 0 \\
		& & & \betaab_{n,n,1} \\
	    \end{bmatrix} \in \R^{(n+1)\times n}, \quad n = 1,2,\dots
\end{align}
