...
%, and since the weight $w_H^{(a+1, b+1)}$ will vanish at $x=0,1$ for non-negative parameters $a,b$, we have that for a polynomial $g(x,y)$ of degree $m$ on $\Omega$: 
%\begin{align*}
%	&\ip<\pddx \Pnkab, g >_{W^{(a+1, b+1)}} \\
%	&= \int_0^1 \int_{-\rho(x)}^{\rho(x)} \pddx f \: g \: W^{(a+1,b+1)} \: \D y \: \D x \\
%	&= \int_{-1}^1 \Big( \int_0^1 \pddx \Pnkab \: g \: w_H^{(a+1,b+1)} \: \rho(x) \: \D x  \Big)  \: w_P^{(b+1)}(y) \: \D y \\
%	&= - \int_{-1}^1 \Big( \int_0^1 \Pnkab \: \pddx \big(g \: w_H^{(a+1,b+1)}(x) \: \rho(x)\big) \: \D x  \Big)  \: w_P^{(b+1)}(y) \: \D y \\
%	&= - \int_{-1}^1 \Big( \int_0^1 \Pnkab \: \pddx \big(g \: \rho(x) \: w_H^{(a+1,b+1)}(x) \: w_P^{(b+1)}(y)\big) \: \D x  \Big) \: \D y \\
%	&= - \int_0^1 \int_{-\rho(x)}^{\rho(x)} \Pnkab \: \pddx \big(g \: \rho(x) \: W^{(a+1,b+1)} \big) \: \rho(x)^{-1} \: \D y \: \D x \\
%	&= - \iint_\Omega \Pnkab \: \big[\pddx g \: W^{(a+1,b+1)} + g \: \pddx W^{(a+1,b+1)} + \: g \rho'(x) \rho(x)^{-1} \: W^{(a+1, b+1)} \big] \: \D y \: \D x \\
%	&= - \ip<\Pnkab,\: \rho'(x) \rho(x)^{-1} \: W^{(1, 1)} \: g + [(a+1)W^{(0, 1)} - 2(b+1)W^{(2, 0)}] \: g + W^{(1, 1)} \: \pddx g>_{W^{(a, b)}} \quad \text{by } (\ref{eqn:weightderivativex}) \\
%	&= - \ip<\Pnkab,\: W^{(2, 0)} \: g + [(a+1)W^{(0, 1)} - 2(b+1)W^{(2, 0)}] \: g + W^{(1, 1)} \: \pddx g>_{W^{(a, b)}} \quad \text{by } (\ref{eqn:rhoderivative})
%\end{align*}
%Now, as the polynomial on the RHS of the inner product is of degree $m+2$ and $\Pnkab$ are orthogonal, we have that the above is zero for $m+2 < n$, i.e $c_{m,j}$ is non-zero for $m = n-1, n-2$ only.
