...
In this section, we limit our discretisation to a single element, that is we let $\element = \Omega$ for a half disk/disk slice/trapezium domain. We can choose our finite dimensional space\sotodo{Use of $h$ is mis-leading here. Maybe $V_p$?} $V_h = \{v_h \in V \quad | \quad {\rm deg}\,(v_h|_\element) \le p\}$ for some $p \in \N$.
...
\begin{align*}
	W^{(a,b,c)}(x,y) &:= w^{(a,b,2c)}_1(x) w^{(c)}_2\fpr(\frac{y}{\rho(x)}) \nonumber \\
