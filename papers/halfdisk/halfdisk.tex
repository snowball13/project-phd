...
We can generalise\sotodo{specialise?} this for our purposes in the following proposition.
\sotodoinline{Can this be simplified to just the definition?}
\begin{proposition}
Let
\begin{align*}
	\Omega := \{(x,y) \in \R^2 \quad | \quad \alpha < x < \beta, \: \gamma \rho(x) < y < \delta \rho(x)\}
...
\end{align*}
Let $f : \Omega \to \R$ be such that $y \mapsto f(x,y)$ for fixed $x$ is an even function. Then,
\begin{align*}
...
\end{align*}
where we achieve equality at $(\star)$ if $y \mapsto f(x,y)$ for fixed $x$ is a polynomial of degree at most $2N-1$ and we achieve equality at $(\star \star)$ if $x \mapsto f(x,y)$ for fixed $y$ is a polynomial of degree at most $N$. Note also that the inner integral at $(\dagger)$ the inner integral over $t$ would be zero if $y \mapsto f(x,y)$ for fixed $x$ was an odd function, due to the symmetry over the domain (i.e. since $\gamma = -\delta$ in the this case).

\bstodoinline{I cannot remember/work out why it was necessary to split f into odd and even parts?}
\end{proof}
...
\sotodoinline{The first part of this section should be moved up to Section 3, and stated as a "Lemma". }
...
\sotodoinline{Comment on how the sparse operators generalise to such domains}
