...
\begin{align*}
	\Omega := \{(x,y) \in \R^2 \quad | \quad \alpha < x < \beta, \: \gamma \rho(x) < y < \delta \rho(x)\}
\end{align*}
with $\rho : (\alpha, \beta) \to \R$. Further, let $\genjacw^{(a,b,c)}(x)$ and $\jacw^{(a,b)}(x)$ be two weight functions on the intervals $(\alpha, \beta)$ and $(\gamma, \delta)$ respectively, given by:
\begin{align*}
\begin{cases}
...
\begin{itemize}
	\item $D_x^{(a,b)}$ has a block-bandwidth of $(-1,2)$, with the blocks themselves having a bandwidth of $(0, 2)$.
  	\item $D_y^{(a,b)}$ has a block-bandwidth of $(-1,1)$, with the blocks themselves having a bandwidth of $(-1,1)$.
	\item $W_x^{(a,b)}$ has a block-bandwidth of $(2,-1)$, with the blocks themselves having a bandwidth of $(2, 0)$.
  	\item $W_y^{(a,b)}$ has a block-bandwidth of $(1,-1)$, with the blocks themselves having a bandwidth of $(1,-1)$.
\end{itemize}
\end{theorem}
...
\begin{align*}
	\Omega := \{(x,y) \in \R^2 \quad | \quad \alpha < x < \beta, \: \gamma \rho(x) < y < \delta \rho(x)\}
\end{align*}
with\sotodoinline{Why $\subset$?} \bstodoinline{I mean to say that $-1 \le \alpha < \beta \le 1$, otherwise this is the half disk case/full disk}
\begin{align*}
\begin{cases}
