...
Further, we note that there exist "parameter transformation" Banded-Block-Banded matrix operators that increment/decrement the parameters, transforming the OPs from one (weighted or non-weighted) parameter space to another. We denote these operators as $T^{(a,b)\to(a+1,b)}$, $T^{(a,b)\to(a,b+1)}$ and $T^{(a,b)\to(a+1,b+1)}$ for conversion between non-weighted spaces, and similarly $T^{(a,b)\to(a-1,b)}$, $T^{(a,b)\to(a,b-1)}$ and $T^{(a,b)\to(a-1,b-1)}$ for conversion between weighted spaces, where:
\begin{align}
...
\begin{align}
    \Delta := D_x^{(1,1)} \: D_x^{(0,0)} + T^{(1,2)\to(2,2)} \: T^{(0,2)\to(1,2)}  \: D_y^{(0,1)} \: D_y^{(0,0)}.
\end{align}

The sparsity of the these operators can be shown using a simple integration by parts argument. We note that the non-zero entries of the differential operators are calculated as follows. 

Note that we can expand $\pddx \Pnkab = \sum_{m=0}^{n} \sum_{j=0}^m c_{m,j} \: P^{(a+1, b+1)}_{m,j}$. The coefficients of these expansion are then the entries of the relevant operator matrix. By a simple integration by parts argument, we have that the only non-zero coefficients of this expansion are when $m = n-1, n-2$, $j = k, k-2$ and $0 \le j \le m$. These are then calculated by taking inner products, i.e. 
\begin{align*}
...
Further, we can expand $\pddy \Pnkab = \sum_{m=0}^{n} \sum_{j=0}^m c_{m,j} \: P^{(a, b+1)}_{m,j}$. The coefficients of these expansion are then the entries of the relevant operator matrix. By a simple integration by parts argument, we have that the only non-zero coefficient of this expansion is when $m = n-1$, $j = k-1$. This is then calculated by taking inner products, i.e. 
\begin{align*}
...
\end{align}
where we note that $\norm{H_n^{(a,b)}}_{w_H^{(a,b)}} = \norm{H_m^{(a,b)}}_{w_H^{(a,b)}}$ $\forall m, n \in \N_0$. 
