...
Three-term recurrences lead to Jacobi operators that correspond to multiplication by $x$ and $y$. Define, for some $N \in \No$: \sotodo{Why bmatrix, not pmatrix?}
\begin{align}
...
\bigPab := \begin{bmatrix}
	...
	\end{bmatrix}
\end{align}
\sotodo{Why not just make $\bigPab$ infinite for now?}and set \(J_x^{(a,b)}, J_y^{(a,b)}\) as the \(\half (N+1)(N+2) \times \half (N+1)(N+2)\) Jacobi matrices corresponding to
\begin{align}
