...
\begin{definition}\label{def:sparsityofdifferentialoperators}
...
The operator matrices $D_x^{(a,b)}, \: D_y^{(a,b)}, \: W_x^{(a,b)}, \: W_y^{(a,b)}$ from Definition \ref{def:sparsityofdifferentialoperators} are sparse, with banded-block-banded structure.
\end{lemma}

\begin{proof}
We proceed with the case for the operator $D_x^{(a,b)}$ for partial differentiation by $x$. Since $\{\hdop^{(a+1, b+1)}_{m,j}\}$ for $m = 0,\dots,n-1$, $j = 0,\dots,m$ is an orthogonal basis for any degree $n-1$ polynomial, we can expand $\pddx \hdopnkab = \sum_{m=0}^{n-1} \sum_{j=0}^m c_{m,j} \: \hdop^{(a+1, b+1)}_{m,j}$. The coefficients of the expansion are then the entries of the relevant operator matrix. We can use an integration-by-parts argument to show that the only non-zero coefficients of this expansion are when $m = n-1, n-2$, $j = k, k-2$ and $0 \le j \le m$. First, note that
\begin{align*}
...
\bstodoinline{Finish this proof!!!}

For the operator for partial differentiation by $y$, we can also expand $\pddy \hdopnkab = \sum_{m=0}^{n-1} \sum_{j=0}^m c_{m,j} \: P^{(a, b+1)}_{m,j}$. The coefficients of the expansion are then the entries of the relevant operator matrix. By a similar argument to before, we have that the only non-zero coefficient of this expansion is when $m = n-1$, $j = k-1$. This is then calculated by taking inner products, i.e. 
\begin{align*}
...
\begin{figure}
	...
	\sotodoinline{Use PDFs for pictures to avoid resolution problems}
	}
        \label{fig:sparsity}
\end{figure}
...
Finally, note that the inner integral at $(\dagger)$ over $t$ would be zero if $y \mapsto f(x,y)$ for fixed $x$ was an odd function, due to the symmetry over the domain (i.e. since $\gamma = -\delta$ in the this case).
\end{proof}
...
\bstodoinline{Does this require more?}
...
\sotodoinline{More Xu references.}
\sotodoinline{Add missing references to Bib file}
