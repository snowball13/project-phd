...
There are of course other approaches for solving PDEs on surfaces, such as the closest point method \cite{macdonald2010implicit, macdonald2008level}. This involves solving the PDE inside a 3D volume that acts as a \enquote{shell} for the surface, and using a \enquote{closest point} to as the solution on the surface. While such an approach is useful for other geometries, it is of low order and does not achieve spectral convergence with sparse discretisations. Further, for our interest of the sphere, it would not take advantage of any rotational symmetry.
...
\begin{figure}[tp]
	...
	\begin{subfigure}[b]{0.55\textwidth}
		\includegraphics[scale=0.4]{condition-numbers-laplacian-N=200}
	\end{subfigure}
	\hfill%
	
	\begin{subfigure}[b]{0.55\textwidth}
		\includegraphics[scale=0.4]{condition-numbers-max-laplacian}
	\end{subfigure}
	\hfill%
	\caption{Plots of the condition number for the Laplacian operator matrix $\Delta^{(1)}_W$ and the preconditioned matrix $P^{-1} \; \Delta^{(1)}_W$ where $P$ is the matrix of the diagonal of $\Delta^{(1)}_W$. Left: The condition numbers of each diagonal block of $\Delta^{(1)}_W$ for $N = 200$, as well as those for $P^{-1} \; \Delta^{(1)}_W$. Right: The maximum condition number of all diagonal blocks of the Laplacian and the preconditioned Laplacian operators, as the degree $N$ increases.}
	\label{fig:conditionnumbers}
\end{figure}

For a given problem $A\vec{x} = \vec{b}$, the condition number is a measure of how accurate solving the system will be (ignoring any numerical rounding errors), and is given as the condition number of the matrix $A$: 
\begin{align*}
	\kappa(A) := \norm{A}_2 \norm{A^{-1}}_2.
\end{align*}
Recall that the Laplacian operator is block-diagonal. One can therefore look at the condition numbers of each block on the diagonal as proxy for the condition number of the Laplacian (and hence the Poisson equation), where graphical representations are seen in \ref{fig:conditionnumbers} for the $\Delta^{(1)}_W$ operator matrix. As the degree $N$ increases, the condition numbers of these blocks becomes increasingly large (at least, the condition number for the first block where $m=0$ does, which will be the largest for each $N$). Fortunately however, by applying a trivial preconditioning matrix similar to that seen for the ultraspherical method \cite{olver2013fast}, we can significantly bound the condition numbers -- this is also seen in \ref{fig:conditionnumbers}. The preconditioning matrix chosen is $P^{-1}$, where $P$ is the matrix of the diagonal of $\Delta^{(1)}_W$.
...
\remark{While it may be possible to leverage a discrete cosine transform (DCT) to speed up the calculation of the coefficients  \cite{slevinsky2018use}, this is an open question.}
