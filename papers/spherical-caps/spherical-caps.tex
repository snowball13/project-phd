...
\end{align*}
and consider the case where $\alpha \in (-1,1)$ and where $ \rho(z) := \sqrt{1-z^2}$.
...
\end{align*}
For simplicity we focus on a spherical cap with   $\beta := 1$, though there is a simple extension to a spherical band by taking $\beta \in (\alpha,1)$. 
