...
\bstodoinline{You mentioned before that you didnt like the use of $i$ being an index throughout (as in $n,k,i$). Is this still the case?}
...
\sotodoinline{This isn't actually shown in the referenced figure, which shows a non-radially symmetric potential} 
\bstodoinline{I dont really understand - isnt $V(x,y,z) =  V(z) = \cos z$ valid? I could be wrong but are you confusing this with the helmholtz example, with a different function $v$? If so, maybe I should make this clearer.}
...
In Figure~\ref{fig:complexity} we plot the time taken\footnote{measured using the \enquote{@belapsed} macro from the BenchmarkTools package in Julia.} \bstodo{cite benchmarktools github?} to construct the operator for $\DeltaS + v(x,y,z)$, this time with a rotationally invariant $v(x,y,z) = \cos z$, and solve a zero boundary condition Helmholtz problem. The plot demonstrates that as we increase the degree of approximation $N$, we achieve a complexity of an optimal $\bigO(N^2)$. \sotodoinline{The example is not the radially symmetric case} \bstodoinline{see other comment on this}
