...
\end{align*}
and consider the case where $\alpha \in (-1,1)$ and where $ \rho(z) := \sqrt{1-z^2}$,  $\beta := 1$ \sotodoinline{Don't we need $\beta \neq 1$ to get a band?} \bstodoinline{yes, but to keep it simple this paper focuses on the cap (otherwise the maths would become more complex/longer. I state that the band is an extension below.}.
...
\end{align*}
noting that $\Wa(x,y,z) = 0$ for $(x,y,z) \in \partial \; \Omega$. The corresponding OPs denoted $\scopnkia(x,y,z)$, where $n$ denotes the polynomial degree, $0 \le k \le n$ and $i \in \{0, \min(1,k)\}$. We define these to be orthogonalised lexicographically, that is, \sotodoinline{I don't understand this ordering. Shouldn't it either be $x^k y^j z^{n-k-j}$ or $x^k y^{j-k} z^{n-j}$?  PS I prefer $j$ to $i$ } \bstodoinline{I have changed to the below. I will change i -> j throughout later if we prefer this.}
\begin{align*}
...
\end{align*}
where $\D \sigma(\xvec, z)$ is the Lebesgue measure on $\Omega$, with
\begin{align}
...
Note that we can define an explicit \(\Dnt\) as follows:\sotodoinline{Improve following notation, say, by adding notation to the top part} \bstodoinline{is this any better?}
\begin{align*}
	\Dnt := 
		\begin{pmatrix}
...
In this section, we will derive the entries of spherical partial differential operators applied to our basis, demonstrating their sparsity in the process. To this end, we will use a different ordering\sotodoinline{I thought we would discuss this separately as a permutation of rows/columns?} \bstodoinline{Not sure what you mean by this comment - maybe I misunderstood before?} of the OPs and coefficients of a function's expansion, in order to exploit the orthogonality the circular harmonics will bring and thus the operators will be block-diagonal. Let $N \in \N$ and recall from equations (\ref{eqn:OPdefNka}, \ref{eqn:OPdefN0a}, \ref{eqn:OPdefNa}) that: 
\begin{align*}
	\bigscopNka &:= 
		\begin{pmatrix}
			\scopa_{k,k,0}(x,y,z) \\
...
\bstodoinline{I have added defns (and sparsity proofs) for two other Laplacian operators. In practice, I cannot get the non-weighted one to work coded up, which I assume is a bug on my end and not the maths (which to me is correct).}
...
For the operator $D_\theta$ for partial differentiation by $\theta$, we simply have that \sotodoinline{Would it be better as ${\rm d} \over {\rm d} \theta$?} \bstodoinline{fixed}
\begin{align*}
...
\end{align*}
Now, note that \sotodoinline{Here and elsewhere: ${\partial \over \partial z}$ is not well-defined as we are on the sphere. Though perhaps ${\rm d} \over {\rm d} z$ works as we are differentiating the formula with respect to $z$?} \bstodoinline{Is the following clearer?}:
\begin{align*}
...
We move onto the spherical Laplacian operators. Note that the Laplacian acting on the weighted and non-weighted spherical cap OP $\scopnki^{(a)}$ yields
\begin{align}
...
For the operator $\calL^{(a)\to(a+\tilde a)}$ for the surface Laplacian on a non-weighted space, the entries of the operator are given by the coefficients in the expansion $\DeltaS \scopnki^{(a)} = \sum_{m=0}^{n} \sum_{j=0}^m \sum_{h=0}^1 c_{m,j,h} \: \scopmjh^{(a + \tilde a)}$, where the coefficients are
\begin{align*}
...
Another desirable operator is the Biharmonic operator $\DeltaS^2$, for which we assume zero Dirichlet and Neumann conditions \sotodoinline{We need to explain Neumann conditions precisely, as the normal derivative is not so obvious}\bstodoinline{Is the following correct? As in, isnt the normal at the boundary still just the unit outward normal on the sphere?}. That is, 
\begin{align*}
	u(x,y,z) &= 0 \quad \text{for } (x,y,z) \in \Omega, \\
	\pfpx{n}{u}(x,y,z) &= \nabla_S u(x,y,z) \cdot \unitvec{n}(x,y,z) = 0 \quad \text{for } (x,y,z) \in \partial\Omega
\end{align*}
where $\partial\Omega$ is the $z=\alpha$ boundary, and $\unitvec{n}(x,y,z)$ is the outward unit normal vector at the point $(x,y,z)$ on the boundary, i.e. $\unitvec{n}(\vec{x}) = {\vec{x} \over \norm{\vec{x}}}$. The matrix operator for the Biharmonic operator will take us from coefficients in the space $\bigW^{(2)}(x,y,z)$ to coefficients in the space $\bigscopN^{(2)}(x,y,z)$. To construct this, we can simply multiply together two of the spherical Laplacian operators defined in \bsrefdef{def:differentialoperators}, namely $\calL^{(0)\to(2)}$ and $\calL_W^{(2)\to(0)}$:
\begin{align*}
	\calB^{(2)}_W := \calL^{(0)\to(2)} \: \calL_W^{(2)\to(0)}.
\end{align*}
Since the operator $\calL_W^{(2)\to(0)}$ acts on coefficients in the $\bigW^{(2)}(x,y,z)$ space, we ensure that we satisfy the zero Dirichlet and Neumann boundary conditions. The sparsity and structure of this biharmonic operator are seen in Figure \ref{fig:sparsity}.
...
\begin{figure}[t]
	...
	\begin{subfigure}{0.5\textwidth}
	\centering
	\includegraphics[scale=0.3]{solutionblocknorms-poisson-N=100-xnolog}
...
\begin{figure}[t]
	...
	\begin{subfigure}{0.5\textwidth}
	\centering
	\includegraphics[scale=0.3]{solutionblocknorms-helmholtz-N=100-xnolog}
        	%\label{fig:solutionblocknorms-poisson}
	\end{subfigure}
...
%
%\sotodoinline{It's already a very long paper so probably better to skip this. But include it in your thesis} \bstodoinline{Yes thought so too}
%
%%\begin{figure}[t]
%%	\begin{subfigure}{0.3\textwidth}
