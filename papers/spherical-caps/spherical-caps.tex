...
\end{align*}
on $\Omega$, where $\D A = \sinphi \: \D \theta \: \D \varphi$ is the Lebesgue measure on $\Omega$. 
\sotodoinline{Is Lebesgue measure the right term? From my understanding the measure of a sphere would be 0} 
\bstodoinline{I have changed it to dA and elaborated a bit more on it, but I am not sure on this so you could be correct that another term needs to be used. Your quadratic surfaces paper with Yuan says Lebesgue I think when discussing this. I have tried to keep it general here by using a measure definition, but we could omit this if its just going to cause confusion?}
\end{proposition}
...
\sotodoinline{The following is wrong as it only applies if $N = \infty$} 
\bstodoinline{What is better? To define them in this way for $N \to \infty$? Or to include the extra terms in this defn below? Or just state that the Jacobi operators approximate multiplication by x,y,z? This also relates to the next comments}
\begin{align}
...
\sotodoinline{Can the following be simplified? Seems like it repeats} 
\bstodoinline{Is it worth removing the previous? And just explaining that we can reorder the Jacobi matrices according to the OP/coefficient vector ordering? This section is important for deriving how to build the OPs, whereas before I simply need to say that the Jacobi operators that apply to coefficients vectors are reorderings of the ones in this section.}
\begin{align}
...
\begin{figure}[t]
	...
	\caption{Complexity plot showing the length of time taken to construct the operator matrix for $\DeltaS + V(x,y,z)$, with rotationally invariant potential $V(x,y,z) = \cos z$. This shows that the time to build and solve the system $\big[\DeltaS + V(x,y,z) \big] \: u(x,y,z) = f(x,y,z)$ is of order $\bigO(N^2)$, where $N$ is the degree to which we approximate the solution. Here, we used $f = -2e^{x}yz(2+x) + (z - \alpha) e^{x} (y^3 + z^2 y - 4xy - 2y)$. \bstodoinline{Redo with belapsed}}
	\centering
...
\sotodoinline{
Wouldn't this trick work for any boundary conditions $c(x,y)$?  That is, expand in Fourier series (a polynomial in $x$ and $y$) and use that to extend off the $z=0$ axis. If you agree we should explain that here.
}
\bstodoinline{Is the following is what you were thinking?}
We can extend this to other boundary conditions that depend only on $x$ and $y$, i.e. $c = c(x,y)$. In this case, the problem
\begin{align*}
	\begin{cases}
    		\DeltaS u(x,y,z) + k^2 \: v(x,y,z) \; u(x,y,z) = f(x,y,z) &\quad \text{in } \Omega \\
...
\begin{align*}
	...
	&= f(x,y,z) + k^2 \: [{1 \over \rho(z)^2} - v(x,y,z)] \: c(x,y)
\end{align*}
for $(x,y,z) \in \Omega$. Notice that the spherical Laplacian applied to $c(x,y)$ is just 
\begin{align*}
	\DeltaS \: c(x,y) = {1 \over \rho(z)^2} \sum_{n=0}^N \sum_{i=0}^1 c_{n,n,i} \: \pmpxm{\theta}{2} \ch_{n,i}(\theta) = -{k^2 \over \rho(z)^2} c(x,y)
\end{align*}
since the coefficients $\{c_{n,k,i}\}$ for such a function are zero for $k < n$ due to the dependence on $x$ and $y$ only.



\subsection{Biharmonic equation}

\sotodoinline{I think we may need to explain what ${\partial u \over\partial n}$ means here: is the normal derivative well defined for 2D surfaces embedded in 3D?}
\bstodoinline{Hopefully expanded on what the normal direction is here, and referenced back to where I defined the Biharmonic operator that also mentions it. Unless I am mistaken, the unit normal is simply the (normalised) positional vector $\vec x$ here. So I dont see a reason why normal derivative wont be well defined (though I very well could be wrong here).}
\bstodoinline{The normal direction (including at the boundary) on a surface is the cross product $\ppx{\varphi}\vec r \times \ppx{\theta}\vec r$ where $\vec r = (x(\varphi,\theta),y(\varphi,\theta),z(\varphi,\theta))^\top$. For the spherical cap its the same as the sphere, i.e. is simply $\rho(z)^2 \: \vec r$ and so the unit normal would be $\vec r (= \vec x)$.}
