...
\bstodoinline{I have added defns (and sparsity proofs) for two other Laplacian operators. In practice, I have found that the non-weighted one is singular - is this correct/expected? The weighted one and the resulting Biharmonic operators both work (as far as I have tested).}
...
\begin{figure}[t]
	...
	\bstodoinline{New examples, such as the suggested $f(\vc x) = \|\vc x - (1,1,1) * (1 /\sqrt 3 + \epsilon)\|$. Though these look like I should remove the trivial examples entirely, as it ruins the scaling.}
	}
	\centering
	\label{fig:helmholtz}
\end{figure}
