...
\input{../includes/bsmacros}
\input{../includes/bsthesisnewcommands}
...
\section{Spherical harmonics as orthogonal polynomials in three variables}
...
\subsection{Defining the spherical harmonics in three variables}
...
for $k \ge 0, \, m \in \Z$. Hence, the expression for multiplication of the spherical harmonic $\Ylmfull$ by $z$ is
\bseqn{
	z\,\Ylm(x,y,z) &= \clm \eimphi \rho(z)^{|m|} z P^{(|m|,|m|)}_{l-|m|}(z)  \\
...
for $l, m \in \No$ s.t. $0 \le m \le l$. Hence, the expressions for multiplication of the spherical harmonic $\Ylmfull$ by $x$ and $y$ is then:
\bseqn{
	x\,\Ylm(x,y,z) &= \clm \cosphi \: \eimphi \: \sintheta \: \rho(z)^{|m|} P^{(|m|,|m|)}_{l-|m|}(z) \\
	...
	y\,\Ylm(x,y,z) &= \clm \sinphi \: \eimphi \: \sintheta \: \rho(z)^{|m|} P^{(|m|,|m|)}_{l-|m|}(z) \\
...
\subsection{Three-term recurrence relation for $\SHvec$}
...
where we note $Y_0^0(x,y,z) \equiv Y_0 := P_0^{(0,0)} \equiv 1$, and for each $l = 0,1,2\dots$,
\begin{align}
	A_l &:= 
		\begin{pmatrix}
			A_{x,l} \\
...
\bstodoinline{quad rule on sphere?}


\subsubsection{Function evaluation}\label{subsubsection:clenshaw}

Once again, let $f(x,y,z)$ be a function on the unit sphere $\Omega$ be approximated by its expansion 
\bseqn{
	f(x,y,z) \approx \SHvec(x,y,z)^\top \fvec = \sum^N_{l=0} \SHvecl(x,y,z)^\top \fvec_l,
}
where $\SHvecl (x,y,z), \fvec_l \in \C^{2l+1}$ for each  $l \in \{0,\dots,N\}$, for some coefficients vector $\fvec$ up to degree order $N \in \N$. We can use the Clenshaw algorithm to evaluate this function at a given point $(x,y,z)$ on $\Omega$ \cite{press2007numerical}. The \textit{Clenshaw algorithm} is then as follows:
\bseqn{
...
The clenshaw algorithm to evaluate $\uvec$ at a point $(x,y,z) \in \Omega$ is then as follows
\bseqn{
	\quad &\text{1) } \text{Set } \vec{\xi}_{N+2} = \bold{0}, \: \vec{\xi}_{N+2} = \bold{0}. \\
...
\bibliography{../includes/thesis-ben-snowball}
