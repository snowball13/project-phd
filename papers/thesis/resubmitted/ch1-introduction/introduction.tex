...
\begin{itemize}
	\item \textit{Define the domain of interest via a restriction of one coordinate in terms of the remainder, that uses a function $\rho$ that satisfies one of two conditions.}
	
		For example, for a domain in 2D space, we would define this as $\Omega := \bsset{(x,y) \in \R^2}{\alpha \le x \le \beta, \: \gamma \rho(x) \le y \le \delta \rho(x)}$.
	
	\item \textit{Express the multivariate and multi-parameter OPs as polynomials in cartesian coordinates.}
	
		The construction of the multivariate OPs takes its inspiration from the Koornwinder approach \cite{koornwinder1975two}. These OPs will be $d$-parameter, meaning that we can construct parameter lowering and raising operators for conversion between the OP bases.
	
	\item \textit{Derive expressions for multiplication of the OPs by the coordinates.}
	
	\item \textit{Use these relations to determine the entries to \enquote{Jacobi matrices} $J_x$, etc. that represent multiplication by the coordinates when applied to the degree-ordered vector of OPs $\bigP$.}
	
		These Jacobi matrices will be banded-block-banded in structure. In fact, due to the construction, they will be block-tridiagonal:
		\bseqn{
			J_{x/y} &= 
			\begin{pmatrix}
				B_{x/y,0} & A_{x/y,0} & & & & \\
	...
	\item \textit{Create a three-term recurrence for the vector of degree $n$ OPs by combining the systems $J_x \bigP = x \bigP$, etc.}
	
		The resulting three-term recurrence will have the form:
		\bseqn{
	...
	\item \textit{Use the multivariate version of the Clenshaw algorithm for evaluation of functions expanded in the OP basis.}
	
	\item \textit{Derive (sparse) relationships for various differential operations, and relations for incrementing and decrementing the parameter values of the OPs. Hence construct the sparse operator matrices representing these operations.}
		
		The differential operators may raise or lower parameter values, which is why the basis conversion operator matrices are required. Further operators may be constructed as combinations of others.
	...
	\item \textit{Use the \enquote{operator Clenshaw algorithm} to construct operator matrices for the action of multiplying by a general function.}
		...
		For instance, this would be useful for a variable coefficient in a Helmholtz example.
\end{itemize}
