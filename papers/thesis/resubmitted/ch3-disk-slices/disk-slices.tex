...
\bstodo{Reworded this paragraph to help make the BC incorporation clearer.}We can tackle the problem as follows. Since $u$ vanishes on the boundary, any polynomial expansion will have $\Wiii(x,y)$ as a factor, and so we can denote the coefficient vector for expansion of $u$ in the $\bigWiii$ OP basis up to degree $N$ by $\vec{u}$, while the coefficient vector for expansion of $f$ in the $\bighdopiii$ OP basis up to degree $N$ we can denote by $\vec{f}$. Since $f$ is known, we can obtain $\vec{f}$ using the quadrature rule above. In matrix-vector notation, our system hence becomes:
\bseqn{
...
\bseqnnumber{
	L(v_p) = a(u_p,v_p) \quad \forall \: v_p \in V_p. \label{eqn:ds:FEMweakform}
}
