...
However, for an arc of the unit circle $\{ define arc \}$ (note we can orientate the arc as we please without loss of generality), we would need to make a modification so that the polynomials are orthogonal with respect to an inner product integral with the truncated domain of the arc:
\bseqn{
	give arc ops (T^h, yU^h) and the inner product.
}
...
This idea of using 1D OPs on a shortened interval to create the modified OPs for an arc has be generalised to higher dimensional domains, where a way of constructing orthogonal polynomials on algebraic curves has been presented \cite{olver2020orthogonal}. \bstodoinline{Expand on this. Summarise the "on" surfaces approach. Use references in the paper.}
...
Using the approach presented for constructing OPs on algebraic curves, we can construct the 3D orthogonal polynomials on the spherical cap or band $\Omega$ from 1D orthogonal polynomials on the interval \([\alpha,\beta]\), and from Fourier series written as orthogonal polynomials in $x$ and $y$:
