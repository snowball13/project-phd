...
If $\uvec$ is a vector-valued function on the tangent space of $\Omega$, it will have the coefficients vectors $\uvec^c, \uvecc, \uveccperp$ for its expansion in the VSH basis up to order $N \in \N$, defined such that
\bseqn{
...
The Clenshaw algorithm presented in \bsrefsection{subsubsection:clenshaw} can also be used to evaluate a vector-valued function in the tangent space for a point $(x,y,z) \in \Omega$, where the matrices $C_n, B_n, \Dnt, G_n(x,y,z)$ are all defined in \bsrefsection{subsection:VSHrecurrence}:
\bseqn{
	\quad &\text{1) } \text{Set } \vec{\xi}_{N+2} = \bold{0}, \: \vec{\xi}_{N+2} = \bold{0}. \\
...
Notice how the iteration also ends at $n=1$ (not $n=0$) here, since the VSHs in our tangent space basis (and hence coefficients) for degree $n=0$ are zero.
...
where $f = 2 \rotrate \cos(\varphi) =  2 \rotrate z$ is the Coriolis parameter, and $\rotrate$ is the rotation rate of the Earth (note that $\varphi = {\pi \over 2} - \text{latitude}$ \bstodo{make a footnote}). On a full scale Earth, the rotation rate is $\rotrate = 7.2921 × 10^{-5}$ rad s$^{-1}$ \bstodo{make a footnote}.
...
For simplicity, we will implement a backward Euler timestepping method to solve the linear SWEs with timestep $\Delta t$:
\bseqn{
