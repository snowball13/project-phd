...
Further, the \textit{associated Legendre polynomials} are a set of polynomials orthogonal with respect to unit weight on the unit interval, and can be defined as the $m^{\text{th}}$ derivative of a Jacobi polynomial: \bstodo{Sheehan - do you know what citation I can use for the definition of the Associated Legendre Polynomials (as the mth derivative of the ordinary Legendre Polynomials)?}
\bseqn{
...
for $m = -l, \dots, -1$. Here, $n! := n \: (n-1) \: (n-2) \dots 1$ for $n \in \N_0$ is the standard factorial, with $0! = 1$. Although referred to as the associated Legendre \enquote{polynomials}, they are not strictly polynomials when $m$ is odd, and as such some authors refer to them as associated Legendre functions instead. Moreover, it should be noted too that the inclusion of the $(-1)^m$ factor in the definition for $m > 0$, known as the \textit{Condon-Shortley phase} in physics, is sometimes omitted.
...
\bstodoinline{Is this paragraph enough? Or do I need to put more detail into explaining these methods, or derive my own?}
...
\bseqnnumber{
	...
	&= \iint_\Omega \cosphi \: \gradYlm \cdot {\gradYlmprime}^* \sinphi \: \D\phi \: \D\theta \nonumber \\
	&= \clm \clmprime \Big( \int_0^{2pi} \eimtheta \eimthetaprime \: \D\theta \Big) \nonumber \\
	&\quadfour \cdot \Big( \int_{-1}^{1} \Big\{ \big[ \abs{m} z \jaclmabs{l}{m}(z) - \rho(z)^2 \jaclmabsdz{l}{m}(z) \big] \big[ \abs{m'} z \jaclmabs{l'}{m'}(z) - \rho(z)^2 \jaclmabs{l'}{m'}(z) \big] \nonumber \\
	&\quadeight + mm' \jaclmabs{l}{m}(z) \jaclmabs{l'}{m'}(z) \Big\} \: z \; \rho(z)^{\abs{m}+\abs{m'}-2} \: \D z \Big) \nonumber \\
	&= \clm \clmprime 2 \pi \delta_{m',m} \: \int_{-1}^{1} \Big\{ z \jaclmabs{l}{m} \jaclmabs{l'}{m'} \rho^{\abs{m}+\abs{m'}-2} \big(mm' + \abs{m}\abs{m'} z^2 \big) \nonumber \\
	&\quadeight \quadtwo - \abs{m} z^2 \jaclmabs{l}{m} \jaclmabsdz{l'}{m'} \rho^{\abs{m}+\abs{m'}} \nonumber \\
	&\quadeight \quadtwo - \abs{m'} z^2 \jaclmabsdz{l}{m} \jaclmabs{l'}{m'} \rho^{\abs{m}+\abs{m'}} \nonumber  \\
	&\quadeight \quadtwo + z \jaclmabsdz{l}{m} \jaclmabsdz{l'}{m'} \rho^{\abs{m}+\abs{m'}+2} \Big\} \: \D z \nonumber \\
	&= \clm c_{l'}^{m} 2 \pi \delta_{m',m} \: \int_{-1}^{1} \Big\{ m^2 z \jaclmabs{l}{m} \jaclmabs{l'}{m} \rho^{2\abs{m}-2} (1 + z^2) \nonumber \\
	&\quadeight \quadtwo - \abs{m} z^2 \jaclmabs{l}{m} \jaclmabsdz{l'}{m} \rho^{2\abs{m}} \nonumber \\
	&\quadeight \quadtwo - \abs{m} z^2 \jaclmabsdz{l}{m} \jaclmabs{l'}{m} \rho^{2\abs{m}} \nonumber  \\
	&\quadeight \quadtwo + z \jaclmabsdz{l}{m} \jaclmabsdz{l'}{m} \rho^{2\abs{m}+2} \Big\} \: \D z \nonumber \\
	&= \clm c_{l'}^{m} 2 \pi \delta_{m',m} \: \int_{-1}^{1} \Big\{ m^2 z \jaclmabs{l}{m} \jaclmabs{l'}{m} \rho^{2\abs{m}-2} (1 + z^2) \nonumber \\
	&\quadeight \quadtwo + \abs{m} \jaclmabs{l'}{m} \rho^{2\abs{m}-2} \big[ z^2 \jaclmabsdz{l}{m} \rho^2 + 2z \jaclmabs{l'}{m} - 2\abs{m}z^3 \jaclmabs{l'}{m} \rho^2 \big] \nonumber \\
	&\quadeight \quadtwo - \abs{m} z^2 \jaclmabsdz{l}{m} \jaclmabs{l'}{m} \rho^{2\abs{m}} \nonumber  \\
	&\quadeight \quadtwo + z \jaclmabsdz{l}{m} \jaclmabsdz{l'}{m} \rho^{2\abs{m}+2} \Big\} \: \D z \nonumber \\
	&= \clm c_{l'}^{m} 2 \pi \delta_{m',m} \: \int_{-1}^{1} \Big\{ \abs{m}(\abs{m}+2) z \jaclmabs{l}{m} \jaclmabs{l'}{m} \rho^{2\abs{m}} \nonumber \\
	&\quadeight \quadtwo + z d_{l,\abs{m}} d_{l',\abs{m}} \jaclmbracket{l}{\abs{m}+1} \jaclmbracket{l'}{\abs{m}+1} \rho^{2\abs{m}+2} \Big\} \: \D z \nonumber \\
	&= \clm c_{l'}^{m} 2 \pi \delta_{m',m} \: \int_{-1}^{1} \Big\{ \abs{m}(\abs{m}+2) \jaclmabs{l'}{m} \rho^{2\abs{m}} \big[ \tilde\gamma_{l,\abs{m},1} \jaclmabs{l-1}{m} + \tilde\gamma_{l,\abs{m},2} \jaclmabs{l+1}{m} \big] \nonumber \\
	&\quadeight \quadtwo + d_{l,\abs{m}} d_{l',\abs{m}} \jaclmbracket{l'}{\abs{m}+1} \rho^{2\abs{m}+2} \big[ \tilde\gamma_{l,\abs{m}+1,1} \jaclmbracket{l-1}{\abs{m}+1} + \tilde\gamma_{l,\abs{m}+1,2} \jaclmbracket{l+1}{\abs{m}+1} \big]\Big\} \: \D z \nonumber \\
	&= \clm 2 \pi \delta_{m',m} \Big\{ c_{l-1}^{m} \delta_{l',l-1} \big[ \abs{m}(\abs{m}+2) {\tilde\gamma_{l,\abs{m},1} \over 2\pi (c_{l-1}^{\abs{m}})^2} + d_{l,\abs{m}} d_{l-1,\abs{m}} {\tilde\gamma_{l,\abs{m}+1,1} \over 2\pi (c_{l-1}^{\abs{m}+1})^2} \big] \nonumber \\
	&\quadfour c_{l+1}^{m} \delta_{l',l+1} \big[ \abs{m}(\abs{m}+2) {\tilde\gamma_{l,\abs{m},2} \over 2\pi (c_{l+1}^{\abs{m}})^2} + d_{l,\abs{m}} d_{l+1,\abs{m}} {\tilde\gamma_{l,\abs{m}+1,2} \over 2\pi (c_{l+1}^{\abs{m}+1})^2} \big] \Big\} \nonumber \\
	&= \clm \delta_{m',m} \Big\{ c_{l-1}^{m} \delta_{l',l-1} \big[ \abs{m}(\abs{m}+2) {\tilde\gamma_{l,\abs{m},1} \over (c_{l-1}^{\abs{m}})^2} + d_{l,\abs{m}} d_{l-1,\abs{m}} {\tilde\gamma_{l,\abs{m}+1,1} \over (c_{l-1}^{\abs{m}+1})^2} \big] \nonumber \\
	&\quadfour c_{l+1}^{m} \delta_{l',l+1} \big[ \abs{m}(\abs{m}+2) {\tilde\gamma_{l,\abs{m},2} \over (c_{l+1}^{\abs{m}})^2} + d_{l,\abs{m}} d_{l+1,\abs{m}} {\tilde\gamma_{l,\abs{m}+1,2} \over (c_{l+1}^{\abs{m}+1})^2} \big] \Big\}. \label{eqn:lemmaVSHrecurrencesproofz}
}
We can see here in \bsrefeqn{eqn:lemmaVSHrecurrencesproofz} that the coefficients $\calC_{l',m'}$ are zero when $l' \ne l \pm 1$ or $m' \ne m \pm 1$, and that the nonzero coefficients are those described in the Lemma as $\Gamma_{l,m,1}$ and $\Gamma_{l,m,3}$.

Next, we also have that
\bseqnnumber{
	&\ip<z \gradYlm, \gradpYlmprime> \nonumber \\
	&= \iint_\Omega \cosphi \: \gradYlm \cdot {\gradpYlmprime}^* \sinphi \: \D\phi \: \D\theta \nonumber \\
	&= \clm \clmprime \Big( \int_0^{2pi} \eimtheta \eimthetaprime \: \D\theta \Big) \nonumber \\
	...
	&= \delta_{l',l} \delta_{m',m} i m (-1)^m. \label{eqn:lemmaVSHrecurrencesproofz2}
}
Again, we can see here in \bsrefeqn{eqn:lemmaVSHrecurrencesproofz2} that the coefficients $\calC^\perp_{l',m'}$ are zero when $l' \ne l$ or $m' \ne m$, and that the nonzero coefficients are those described in the Lemma as $\Gamma_{l,m,2}$.
...
As for the scalar case, these coefficients detailed above are then the nonzero entries of the Jacobi matrices for the tangent space basis. Define $\VSHvecfull$ by
\bseqnnumber{
	\VSHvecfull = 
		\begin{pmatrix} 
...
\bseqn{
	\begin{cases}
		...
		\VSHvecfull_{l+1}(x,y,z) = -\Dlt (B_l-G_l(x,y,z)) \SHvec_l(x,y,z) - \Dlt C_l  \,\SHvec_{l-1}(x,y,z), \quad l \in \N.
	\end{cases}
}
...
\bseqn{
	...
	\big(\vec{\eta}_{1}\big)_j &= 
		\begin{cases}
			\big(\vec{\eta}_{0}\big)_j & j = 2, 4(2l+1) + 4 \\
			0 & o/w
		\end{cases}, \\
	...
	\big(\vec{\eta}_{3}\big)_j &= 
		\begin{cases}
			\big(\vec{\eta}_{2}\big)_j & j = 2(2l+1), 6(2l+1) - 4 \\
			0 & o/w
		\end{cases}.
}
For $l = 0$ we can simply set
...
\bseqn{
	\uvec(x,y,z) \approx \VSHvecfull(x,y,z)^\top \uvec^c := \sum^N_{l=0} \sum^l_{m=-l} \big[u^{\gradY}_{l,m} \, \gradYlm(x,y,z)  + u^{\gradpY}_{l,m} \, \gradpYlm(x,y,z) \big].
}


\subsubsection{Obtaining coefficients}
\bstodoinline{As for the scalar case, do I need to say more here about the transforms?}
Just like for the scalar case, we would like to be able to obtain the coefficients for the expansion of a vector valued function in the VSH basis up to a given degree. To do this, we can use a vector spherical harmonic transform. The development of vector spherical harmonic transforms is well established \cite{swarztrauber1993vector}, and continues today (see \cite{gia2019favest}). 
...
\begin{proof}
	The argument for $\calG$ is trivial by definition. Now, the spherical harmonics, as the name eludes to, satisfy a harmonic relationship $\DeltaS \Ylm = \: l(l+1) \Ylm$ (see e.g. \cite{humi1987factorisation}). Thus, for the proof for $\calD$, consider
\bseqn{
...
\bseqn{
	f \: \rvec \times \uvec(x,y,z) &= \VSHvec(x,y,z)^\top \: \calF \: \uveccperp + \Big(\VSHvecperp(x,y,z)\Big)^\top \: \calF^\perp \: \uvecc
}
\end{definition}
\begin{lemma}
...
\bseqn{
	f \: \rvec \times \bold{u} &= f \: \rvec \times \sum_{l=0}^{N} \sum_{m=-l}^{l} [ u^{\gradY}_{l,m} \gradYlm + u^{\gradpY}_{l,m} \gradpYlm ] \\
	&= 2 \: \rotrate \: z \: \sum_{l=0}^{N} \sum_{m=-l}^{l} [ u^{\gradY}_{l,m} \rvec \times \gradYlm + u^{\gradpY}_{l,m} \rvec \times ( \rvec \times \gradYlm ) ] \\
	&= 2 \: \rotrate \: \sum_{l=0}^{N} \sum_{m=-l}^{l} [ u^{\gradY}_{l,m} z \: \gradpYlm - u^{\gradpY}_{l,m} z \: \gradYlm ) ] \\
	&= 2 \: \rotrate \: \sum_{l=0}^{N} \sum_{m=-l}^{l} \Big[ u^{\gradY}_{l,m} \: \Big( \Gamma_{l,m,1}^* \gradpY^{m}_{l-1} + \Gamma_{l,m,3}^* \gradpY^{m}_{l+1} + \Gamma_{l,m,2} \gradY^{m}_{l} \Big) \\
	&\quad \quad \quad \quad \quad \quad \quad \quad - u^{\gradpY}_{l,m} \: \Big( \Gamma_{l,m,1} \gradY^{m}_{l-1} + \Gamma_{l,m,3} \gradY^{m}_{l+1} + \Gamma_{l,m,2}^* \gradpY^{m}_{l} \Big) \Big].
}
...
\bseqn{
	\uvecc_{n+1} &= \uvecnc + \Delta t \: (\calG \vec{h}^c_{n+1} - \calF \uveccperp_{n+1}) \\
	\uveccperp_{n+1} &=  - \Delta t \: \calF^\perp \uvecc_{n+1} \\
	\vec{h}^c_{n+1} &= \vec{h}^c_n - \Delta t \: \calH D \uvecc_{n+1}
}
$\iff$
\bseqn{
	\uvecc_{n+1} &= \uvecc_{n} + \Delta t \: (\calG \vec{h}^c_n - \Delta t \: \calH \calG \calD \uvecc_{n+1} + \Delta t \calF \calF^\perp \uvecc_{n+1}) \\
	\uveccperp_{n+1} &=  - \Delta t \: \calF^\perp \uvecc_{n+1} \\
	\vec{h}^c_{n+1} &= \vec{h}^c_n - \Delta t \: \calH D \uvecc_{n+1}
}
$\iff$
\bseqn{
	\uvecc_{n+1} &= (I + \Delta t^2 \calH \calG \calD - \Delta t^2 \calF \calF^\perp)^{-1} \: (\uvecc_{n} + \Delta t \: \calG \vec{h}^c_n) \\
	\uveccperp_{n+1} &=  - \Delta t \: \calF^\perp \uvecc_{n+1} \\
	\vec{h}^c_{n+1} &= \vec{h}^c_n - \Delta t \: \calH \calD \uvecc_{n+1}.
}
Here, it is clear that the matrix $I + \Delta t^2 \calH \calG \calD - \Delta t^2 \calF \calF^\perp$ is sparse with banded-block-banded structure, and hence this system will be efficient to solve. Doing so at each iteration provides us with the coefficients vectors $\uvecc_{n+1}, \uveccperp_{n+1}, \vec{h}^{c}_{n+1}$ if we know $\uvecc_{n}, \uveccperp_{n}, \vec{h}^{c}_{n}$. 

\bstodo{It would be nice to have some images here. However I need to update/fix/tidy my SH code as its probably got a few bugs in it having now gone back over the derivations that I have done.}
