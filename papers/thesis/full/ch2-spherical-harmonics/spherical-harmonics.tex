...
It is useful to be able to transform between these cartesian coordinates and the spherical coordinates ($\varphi, \theta$). Throughout this thesis we will use the convention that the spherical coordinate angles be defined by
\bseqn{
...
The framework we have outlined for the spherical harmonics and the vector spherical harmonics allows us to easily and explicitly derive sparse operator matrices for partial differential operators such as the spherical gradient, divergence and Laplace--Beltrami, as well as other key operators useful in examples such as the linearised shallow water equations. These operators will take coefficients of a function's expansion in either the SSH or VSH basis, to coefficients in either basis. 
...
\begin{figure}[t]
	\centering % <-- added
	\centerline{\includegraphics[scale=0.5]{linear-swe-H=1-dt=0p01-nsteps=100-arrows}}
