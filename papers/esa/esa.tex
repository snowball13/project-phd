...
\sotodoinline{Outline the "big picture" here. Say that the ultimate goal is to solve PDEs on spherical triangles, and what is listed below are steps on the way to that goal.}

This document is to provide a brief description of my research area which will be the topic of my thesis. All code used thus far and from hence forth will mainly be written in the Julia programming language.
...
\end{align} 
Note that we can then express \(\Ylm\) in terms of \(x,y,z\) instead of \(\theta, \varphi\) by noting that \(\costheta = z\) and that \( \eimphi\) can be expressed in terms of \(x,y,z\) for any \(m\in\mathbb{Z}\). Indeed, they are polynomials in $x,y,z$ which we denote $\Ylm(x,y,z)$. They span all polynomials modulo the ideal generated by $x^2 + y^2 + z^2 - 1$. \sotodo{Mention something like: this is equivalent to standard techniques in the literature on spherical harmonics, but the goal here is to put it in a language that will translate to spherical triangles}

\citep{dunkl2014orthogonal} provided a framework for gaining a recurrence relation for each \(\bigP_{l+1}\) in terms of the previous two sub-vectors \(\Pl\) and \(\bigP_{l-1}\), utilising the sub matrices that make up the block-tridiagnonal Jacobi operator matrices for multiplication by $x, y$ and $z$. Clenshaw's algorithm then gives us a way of evaluating a function given its expansion in the spherical harmonic basis (up to a finite order).  \sotodo{Mention that "this provides an optimal complexity method for evaluating spherical harmonic expansions. This is an alternative to using the explicit expressions that may have benefits, though further investigation is required."}
...
\end{align}
where \(\Alm, \Blm, \Dlm, \Elm, \Flm, \Glm \in \R\) have derivable and calculable formulae.\sotodo{Mention that the formulae are explicit}
...
We can use the Clenshaw algorithm \sotodo{I would say "a multivariate analogue of Clenshaw's algorithm}  to evaluate a function at a given point \((x,y,z)\) on the unit sphere provided we know the coefficients of the function when expanded in the spherical harmonic basis, i.e. suppose \(f(x,y,z)\) is a function and we know the set \(\{\bold{f}_l\}\) s.t.
\begin{align}
...
By noting that the tangent space is then spanned by\sotodo{Connect these to "vector spherical harmonics"} $\{\gradYlm, \gradpYlm\}$, and that these gradients and perpendicular gradients are orthogonal polynomials themselves, we can use the same ideas to create a framework for functions in the tangent space as expansions in this OP basis.
...
Then using the operators on the coefficient vectors, the timestepping method is: \sotodo{Explain what $F$, $K$ , and $G$ are and emphasise they can all be constructed}
\begin{align}
...
As a stepping stone to working on spherical triangles, we will try to construct orthogonal polynomials on a circle arc, given by $x \in [h, 1]$, $y = (1-x^2)^\frac{1}{2}$. We will use the lanczos algorithm from\sotodo{"as implemented", not "from"} ApproxFun \cite{ApproxFun} to obtain 1D OPs $\{T^h_n\}$ (orthogonal wrt $w(x) = (1-x^2)^{-\frac{1}{2}}$) and $\{U^h_n\}$ (orthogonal wrt $w(x) = (1-x^2)^{\frac{1}{2}}$) on the interval $[h,1]$, and create our 2D OPs according to:\sotodo{Cite Huybrechs reference for connection in literature}
\begin{align}
...
By using a similar process to\sotodo{Try to always include page numbers in citations to books, as otherwise you'll forget where it is} \cite{dunkl2014orthogonal} we can construct 2D orthogonal polynomials on the half disk from 1D orthogonal polynomials on the intervals \([0,1]\) and \([-1,1]\). We denote the half disk by \(\Omega\) where
\begin{align}
...
We can obtain the matrix operator for \(\Delta\), that will take us from coefficients for expansion in the weighted space \sotodo{Mention this vanishes on the boundary so imposes zero Dirichlet}
\[
\Wii(x,y) \: \Pii(x,y)
\]
to coefficients in the non-weighted space\sotodo{The notation $\Pii$ is not defined I think}
\[
\Pii(x,y).
\]
This operator will have Banded-Block-Banded structure. We first note that, for general \(a,b\) and general function \(f(x,y) = \mathbf{f}^T \: \bigPab(x,y)\), there exist Banded-Block-Banded matrix operators \(D_x^{(a,b)}, \: D_y^{(a,b)}, \: W_x^{(a,b)}, \: W_y^{(a,b)}\) such that: \sotodo{Mention this is analoguous to Jacobi polynomials, DLMF ...}
\begin{align}
...
\end{align}
Then, the operator \(\Delta\) is given by \sotodo{Mention it is also Banded-Block-Banded, hence sparse}
\begin{align}
...
The goal here is to eventually investigate sparse spectral methods on triangulations of the sphere as an alternative to the spherical harmonics approach currently in use at the European Centre for Medium-range Weather Forecasts (ECMWF) in their weather and climate model. The aim would be to preserve the predictive skill of the current approach whilst avoiding the parallel scalability bottleneck from the global spectral transform, which is expected to inhibit the future performance of the ECMWF model. \sotodo{This last paragraph could be in the intro}


\sotodoinline{Maybe mention that isosceles triangles should follow by the same procedure as the half sphere, but general spherical triangles will require further ideas.}


\sotodoinline{Expand bibliography: cite spherical harmonics books, Huybrechs, ECMWF paper, etc.}
