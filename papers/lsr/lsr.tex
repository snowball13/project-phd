...
\begin{align}\label{eqn:domaindef}
...
For simplicity of presentation we focus on the disk-slice, where $\rho(x) = \sqrt{1-x^2}$, $(\alpha,\beta) \subset (0, 1)$, and $(\gamma, \delta)  = (-1,1)$, and discuss an extension to the end-disk-slice in the appendix (note we omit detailed discussion on the extension to the trapezium in this document). 
...
The OPs $\{\hdopnk^{(a,b,c,d)}\}$ are orthogonal with respect to the weight
\begin{align*}
...
\begin{align*}
	\alphaabcd_{n,k,1} &:= \beta_{n-k-1}^{(a, b+k+\half)}, \qquad \alphaabcd_{n,k,2} := \alpha_{n-k}^{(a, b+k+\half)} \\
...
\subsubsection{Obtaining the coefficients for expansion of a function on the disk-slice}
...
\section{Orthogonal Polynomials on spherical caps and spherical triangles (3D)}
...
One approach would be to build the 3D OPs on the spherical cap by in a similar vein as to how we could build the circle arc OPs, by projecting up from the 2D orthogonal polynomials on the disk to 3D OPs on the sphere. The approach that we will first consider however will involve developing the 3D OPs on the spherical cap in a similar way to how we developed the 2D OPs on the disk-slice, similar to the approach of \cite{olver2019orthogonal}.

Define the spherical cap, denoted by $\Omega$, by
\begin{align}
	\Omega := \{(\xvec, t) \in \R^3 \quad | \quad t \in (\alpha, 1), \: \xvec \in \R^2, \: ||\xvec|| = \rho(t)\}.
\end{align}
where $0 \le \alpha < 1$ and $\rho(t) := (1 - t^2)^\half$.

Let $\genjacw^{(a,b)}(t)$ a weight function on the interval $(\alpha, 1)$ given by:
\begin{align*}
	\genjacw^{(a,b)}(t) &:= (t - \alpha)^{a} \: \rho(t)^{b}
\end{align*}
and define the associated inner product by:
\begin{align}
	\ip< p, \: q >_{\genjacw^{(a,b)}} &:= \frac{1}{\normgenjac^{(a,b)}} \: \int_\alpha^\beta p(t) \: q(t) \: \genjacw^{(a,b)}(t) \: \D t \label{eqn:ipgenjactwoparam}
\end{align}
where
\begin{align}
	\normgenjac^{(a,b)} := \int_\alpha^\beta \: \genjacw^{(a,b)}(t) \: \D t.
\end{align}
Denote the two-parameter family of orthonormal polynomials on $[\alpha,1]$ by $\{\genjac_n^{(a,b)}\}$, orthonormal with respect to the inner product defined in (\ref{eqn:ipgenjactwoparam}).

Let $\{Y_{m,k}\}$ for $m = 0, 1, \dots$, $k = 0, 1$ be the 2D OPs known as the circular harmonics on the unit circle; that is, if we define $\theta$ by $x = \cos(\theta)$, $y = \sin(\theta)$ for $\xvec := (x, y)$ where $||\xvec|| = 1$,
\begin{align}
	Y_{m,0}(\xvec) = Y_{m,0}(x, y) := \cos(m \theta), \quad Y_{m,1}(\xvec) = Y_{m,1}(x, y) := \sin(m \theta)
\end{align}
orthonormal with respect to the inner product
\begin{align}
	\ip< p, \: q >_{Y} &:= \frac{1}{\sqrt{\pi}} \: \int_{||\xvec|| = 1} p(\xvec) \: q(\xvec) \: \D \sigma(\xvec) = \frac{1}{\sqrt{\pi}} \: \int_0^{2\pi} p(\xvec(\theta)) \: q(\xvec(\theta)) \: \D \theta. \label{eqn:ipcircularharmonics}
\end{align}
We can then define the 3D family of OPs on $\Omega$ as the set of polynomials $\{\Qab_{n,m,k}\}$ where
\begin{align}
	\Qab_{n,m,k}(\xvec, t) := \genjac_{n-m}^{(a,2b+2m+1)}(t) \: \rho(t)^m \: Y_{m,0}\fpr(\frac{\xvec}{\rho(t)})
\end{align}
orthogonal on $\Omega$ with respect to
\begin{align}
	\ip< p, \: q >_{\Qab} &:= \int_\Omega p(\xvec, t) \: q(\xvec, t) \: \genjacw^{(a,2b)}(t) \: \D \sigma(\xvec, t) \nonumber \\ 
	&= \int_\alpha^1 \int_{||\xvec|| = \rho(t)} p(\xvec, t) \: q(\xvec, t) \: \D \sigma(\xvec) \: \genjacw^{(a,2b)}(t) \: \D t \nonumber \\
	&= \int_\alpha^1 \rho(t) \int_{||\xvec|| = 1} p(\rho(t) \xvec, t) \: q(\rho(t) \xvec, t) \: \D \sigma(\xvec) \: \genjacw^{(a,2b)}(t) \: \D t. \label{eqn:ipsphericalcaps}
\end{align}
...
We have shown that bivariate orthogonal polynomials can lead to sparse discretizations of general linear PDEs on specific domains whose boundary is specified by an algebraic curve---notably here the disk-slice---with Dirichlet boundary conditions. This work is related to the triangle case \cite{olver2019triangle}, and forms a building block in developing an $hp-$finite element method to solve PDEs on other polygonal domains by using suitably shaped elements, for example, by dividing the disk into disk slice elements. Other extensions to the disk-slice work could include a full $hp$-finite element method on sections of a disk, which has applications in turbulent pipe flow.

Moving forward, this work serves as a stepping stone to constructing similar methods to solve partial differential equations on 3D sub-domains of the sphere, such as spherical caps and spherical triangles. In particular, orthogonal polynomials (OPs) in cartesian coordinates ($x$, $y$, and $z$) on a spherical cap can be represented using two families of OPs on the end-disk-slice (see \cite[Theorem 3.1]{olver2018orthogonal} for a similar construction of OPs on an arc in 2D), or from a rotation framework involving the non-classical 1D OPs defined in this document and circular harmonics (see \cite{olver2019orthogonal} for a similar construction of OPs in other quadratic surfaces of revolution). Further, we aim to show that the latter's construction leads to discretizations of spherical gradients and Laplacian's that are sparse on half-spheres and other suitable sub-components of the sphere. The resulting sparsity in high-polynomial degree discretizations presents an attractive alternative to methods based on bijective mappings (e.g., \cite{DGShallowWater,FEMShallowWater,boyd2005sphere}). Constructing these sparse spectral methods for surface PDEs on half-spheres, spherical caps, and spherical triangles is the next steps of my PhD, which has applications in weather prediction \cite{staniforth2012horizontal}. 
